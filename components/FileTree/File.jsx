...
const File = ({ fileName, selectContent, setCurrentContent, contentType, host, editable, topic }) => {
    ...
    return (
        !removed?<li onDrag={e=>{console.log(e)}}>{
            (isEditing) ? <input onChange={e => { setFname(e.target.value) }} onBlur={async e => {
                ...
                color: "#555",
                width: "100%"
            }} value={fname} />: <div  onClick={e => {
                selectContent($(e.target)); setCurrentContent([topic?(topic + "/"):"" + fname, contentType.toLowerCase().slice(0, -1)]);
